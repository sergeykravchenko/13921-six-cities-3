...
import withRatingValues from "../../hocs/with-rating-values/with-rating-values";
...
interface Props {
  ...
  rating: number;
  onRatingChange: () => void;
  onSubmit: (id: idType, {rating, comment}: {rating: number; comment: string}) => void;
  onReset: () => void;
...
class ReviewsForm extends React.PureComponent<Props, null> {
  ...
  _createRating() {
    const {rating, onRatingChange, requestStatus} = this.props;
